      ...
      <motion.div 
        animate={{ 
          background: `radial-gradient(circle at 50% 0%, ${
            selectedMood === 'happy' ? '#ec4899' : 
            selectedMood === 'angry' ? '#ea580c' : 
            selectedMood === 'manic' ? '#eab308' : 
            '#6366f1'
          } 0%, transparent 70%)` 
        }}
          ...
          <div className="flex gap-3 overflow-x-auto py-4 pb-4 -mx-6 px-6 scrollbar-hide [&::-webkit-scrollbar]:hidden [-ms-overflow-style:none] [scrollbar-width:none]">
            {moods.map((m) => (
              <button
                ...
                className={`flex flex-col items-center gap-2 p-3 rounded-2xl border transition-all duration-300 min-w-[70px] ${
                  selectedMood === m.id 
          ...
          >
            ...
            <div className="relative z-10">
              ...
              <h3 className="text-xl font-bold text-white mb-2">
                {selectedMood === 'angry' ? "Dinginkan Kepala" :
                 selectedMood === 'manic' ? "Salurkan Energimu" :
                 selectedMood === 'happy' ? "Abadikan Momen" : "Tetap Fokus"}
              </h3>
              <p className="text-sm text-slate-300 mb-4 leading-relaxed">
                {selectedMood === 'angry' ? `Terdeteksi stres tinggi. Coba teknik napas 4-7-8 sekarang.` :
                 selectedMood === 'calm' ? "Mood yang pas untuk produktif. Lanjutkan tugas codingmu." :
